...
sub Mojo::Weixin::_webwxgetcontact {
	...
	my $callback = sub {
		...
		$self->warn("getcontact本次获取MemberCount为".$json->{MemberCount});
		if (($self->is_update_all_friend and defined $json->{Seq} and $json->{Seq} != 0) or ($continueget==1)){
			#获取的不全，需要继续获取其余部分
			my @query_string = (
				r           =>  $self->now(),
				seq         =>  $seq,
            ...
            $flag = 1 ;
            $seq = $json->{Seq};
			$self->warn("getcontact继续获取联系人");
			
			my $callback0= $self;
			my $id = $self->http_get($self->gen_url($api,@query_string),{Referer=>'https://'.$self->domain . '/',json=>1, ua_retry_times=>1, ua_connect_timeout=>30,ua_request_timeout=>60,ua_inactivity_timeout=>30}, $callback0);
		...
		#异步之后不能批量调用
		$self->add_friend(Mojo::Weixin::Friend->new($_)) for @friends;
		$self->add_group(Mojo::Weixin::Group->new($_)) for @groups;
	};
	...
	$self->info("开始获取".$api);
	my $json = $self->http_get($self->gen_url($api,@query_string0),{Referer=>'https://'.$self->domain . '/',json=>1, ua_retry_times=>1, ua_connect_timeout=>30,ua_request_timeout=>30,ua_inactivity_timeout=>30}, $callback);
    ...
    do {
        ...
        $self->info("开始获取".$api);
        my $json = $self->http_get($self->gen_url($api,@query_string),{Referer=>'https://'.$self->domain . '/',json=>1, ua_retry_times=>1, ua_connect_timeout=>30,ua_request_timeout=>30,ua_inactivity_timeout=>30});
        $self->info("获取_webwxgetcontact的API结束");
